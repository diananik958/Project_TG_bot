...
async def end(call: types.CallbackQuery, state: FSMContext):
    ...
    if str(answer) == "1":
        ...
        await call.message.answer(
            text="Следующий вопрос: /onstarttest\nВаш ответ на вопрос №1:\n "
...
async def tower(call: types.Message, state: FSMContext):
    b = towers()
    answer = call.text
    await state.update_data(name=answer)
    await call.answer(text="Вопрос №2\nВ каком месте вы можете столкнуться с фишинговой ссылкой? \nВыберите номер ответа\n"
...
async def end(call: types.CallbackQuery, state: FSMContext):
    ...
    if str(answer) == "1":
        ...
        await call.message.answer(
            text="Тест закончен", reply_markup=ReplyKeyboardRemove())
        RESULT.append(0)
    print(RESULT)
    if sum(RESULT) > 5:
        await call.message.answer(
